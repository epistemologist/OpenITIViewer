...
from defs import Text, Match, all_texts, all_primary_texts, all_text_tags, texts_by_filename
...
def search_all_files(
	...
	filenames = [f"./openiti_raw/{text.filename}.raw" for text in 
		(all_texts if not search_options.only_primary_version else all_primary_texts)
		if len(text.tags & search_options.text_tags) > 0
	]
	print(filenames)
	if verbose:
